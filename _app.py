...
def get_item_by_id(id):
    try:
        ...
        db, cursor = x.db()
        # Create a query
        q = "SELECT * FROM users WHERE user_pk = %s" #placeholder = %s
...
def update_user(user_pk):
    try:
        ...
        q = f"""
            UPDATE users
            SET {user_pk, user_name, user_last_name}
            WHERE user_pk = %s
        """
        print(user_pk, user_name, user_last_name, flush=True)
        ...
        return f"""
        <browser mix-update="#right" mix-fade-2000>
            {user_html}
            {user_more_html}
            </browser>
        """
